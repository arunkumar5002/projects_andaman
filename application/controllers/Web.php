...
class Web extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['category_types'] = $this->package_model->type_list();
		/* echo"<pre>";
	print_r($data['banner']);
	exit(); */
	...
	public function package_list($package_title = null, $pack_id = null, $id = null)
	{
		//$package_title = urldecode($this->uri->segment(3));
		$this->session->set_userdata('pack_id', $package_title);
		/* echo "<pre>";
		  print_r($id);
		  exit();  */
		$data['type'] = $this->package_model->create_list();
	...
	public function package_type($package_title, $package_type)
	{
		$data['type'] = $this->package_model->create_list();
		$data['category_types'] = $this->package_model->type_list();
		$data['category'] = $this->package_model->get_type();
		$data['packages'] = $this->web_model->get_package_by_title_and_type($package_title, $package_type);
		$data['pack'] = $this->package_model->get_packages();
	...
	public function filter_list($pack_id, $type_id)
	{
		$data['type'] = $this->package_model->create_list();
		$data['category_types'] = $this->package_model->type_list();
		$data['category'] = $this->package_model->get_type();
		$data['pack'] = $this->package_model->get_packages();
		$data['packages'] = $this->package_model->get_packages_by_type($pack_id, $type_id);
		$data['content'] = "web/destinations";
		$this->load->view('web/template', $data);
	}
